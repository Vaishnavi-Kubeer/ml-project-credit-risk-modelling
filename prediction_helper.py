...
def prepare_df(age,income,loan_amount,loan_tenure_months,avg_dpd_per_delinquency,
            delinquency_ratio,credit_utilization_ratio,num_open_accounts,residence_type,loan_purpose,
            loan_type):
...
def calculate_credit_score(input_df,base_score=300,scale_length=600):
    ...
    #sigmoid funtion for probability
    default_probability=1/(1+np.exp(-x))
    non_default_probability=1-default_probability
    # formula for credit score
    credit_score=base_score+non_default_probability.flatten()*scale_length
    # as per business rules determine rating category
    def rating(score):
        if 300<=score<500:
            return 'Poor'
        elif 500<=score<650:
    ...
    rating_=rating(credit_score[0])
    return default_probability.flatten()[0],int(credit_score),rating_
